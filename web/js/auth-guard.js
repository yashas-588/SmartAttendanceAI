...
async function verifyLocation(session) {
  ...
  // Fallback / migration check: if session has the coordinates, use them!
  if (session && session.location) {
    location = session.location;
    allowedRadius = (session.location_radius !== undefined && session.location_radius !== null) ? Number(session.location_radius) : 10;
    if (allowedRadius === 0) {
    ...
    // Legacy session or no session passed -> fall back to classroom settings!
    try {
      const settingsDoc = await db.collection('settings').doc('classroom').get();
      if (settingsDoc.exists) {
        const cfg = settingsDoc.data();
      ...
      console.warn('Failed to load geofence fallback settings:', e);
    }
  }

  if (geofenceDisabled || !location) {
    return { passed: true, classroomSet: true, reason: 'Geofence disabled (radius = 0m)' };
  }
  ...
  if (navigator.permissions && navigator.permissions.query) {
    ...
    console.log(`[GPS_TRACE] navigator.permissions.query not supported`);
  }
  ...
  return new Promise(resolve => {
    ...
    const attemptGPS = (highAccuracy) => {
      console.log(`[GPS_TRACE] Before Geolocation request: highAccuracy=${highAccuracy}, timeout=30000ms`);
      navigator.geolocation.getCurrentPosition(
        pos => {
          console.log(`[GPS_TRACE] Success Callback executed.`);
          console.log(`[GPS_TRACE] Latitude: ${pos.coords.latitude}`);
          console.log(`[GPS_TRACE] Longitude: ${pos.coords.longitude}`);
          console.log(`[GPS_TRACE] Accuracy: ${pos.coords.accuracy}m`);
        ...
        err => {
          console.error(`[GPS_TRACE] Error Callback executed. Code: ${err.code}, Msg: ${err.message}`);
